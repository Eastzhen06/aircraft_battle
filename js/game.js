...
class Game {
    constructor() {
        ...
        // 云端存档会在登录后异步加载（见 loadCloudSaveToGame 方法）
    ...
    init() {
        ...
        // 检查是否已有登录态，有则跳过登录界面
        authGetUser().then(async (user) => {
            if (user) {
                if (user.email && user.email.endsWith('@local.username')) {
                    this.currentUsername = user.email.replace('@local.username', '');
                }
                await this.loadCloudSaveToGame();
    ...
    // 从云端加载存档并同步到游戏实例和 localStorage（合并策略，不丢本地解锁）
    async loadCloudSaveToGame() {
        if (localStorage.getItem('os2_dev_mode') === 'true') {
            console.log('[SYNC] Dev mode 激活中，跳过云端同步');
            this.populateLevelGrid();
            this.renderHangar();
            return;
        }
        try {
            ...
            if (!cloudSave) {
                // 云端没有存档（新账号或历史遗留），从本地回写到云端
                console.log('[SYNC] 云端无存档，从本地回写...');
                await saveToCloud({
                    unlocked_planes: this.unlockedPlanes,
                    unlocked_wingmen: this.unlockedWingmen,
                    max_cleared_level: this.maxClearedLevel
                });
                return;
            }

            // 云端有数据 — 云端权威，直接覆盖本地（不合并、不回写）
            if (cloudSave.unlocked_planes && cloudSave.unlocked_planes.length > 0) {
                this.unlockedPlanes = cloudSave.unlocked_planes;
                localStorage.setItem('os2_unlocked_planes', JSON.stringify(this.unlockedPlanes));
            }
            if (cloudSave.unlocked_wingmen && cloudSave.unlocked_wingmen.length > 0) {
                this.unlockedWingmen = cloudSave.unlocked_wingmen;
                localStorage.setItem('os2_unlocked_wingmen', JSON.stringify(this.unlockedWingmen));
            }
            if (cloudSave.max_cleared_level != null) {
                this.maxClearedLevel = cloudSave.max_cleared_level;
                localStorage.setItem('os2_max_cleared_level', String(this.maxClearedLevel));
            }
            ...
            console.warn('[SYNC] 云端存档加载失败，使用本地存档:', e);
        }
    }

    // 用户按钮：显示/隐藏 + 点击打开修改密码弹窗
    setupUserButton() {
        const userBtn = document.getElementById('user-btn');
        const userNameSpan = document.getElementById('user-btn-name');
        if (!userBtn) return;

        if (this.currentUsername && !this.isGuestMode) {
        ...
        newUserBtn.addEventListener('click', () => {
            document.getElementById('cpwd-username').value = this.currentUsername;
    ...
    setupEventListeners() {
        ...
        document.getElementById('dev-submit')?.addEventListener('click', async () => {
            ...
            if (acc === '0002' && pwd === '1110') {
                ...
                localStorage.setItem('os2_dev_mode', 'true');
                alert("[OS2_SYS] 鉴权通过。全级图鉴与全关卡物理挂载完毕。（仅本地生效）");
                location.reload();
            } else {
        ...
        document.getElementById('auth-register-btn')?.addEventListener('click', async () => {
            ...
            const password = document.getElementById('auth-password').value;
            if (!username || !password) { authError.textContent = '请输入用户名和密码'; return; }
            if (username.length < 3) { authError.textContent = '用户名至少3个字符'; return; }
            if (password.length < 6) { authError.textContent = '密码至少6个字符'; return; }
        ...
        // 修改密码弹窗事件
        // ==========================================
        document.getElementById('cpwd-cancel')?.addEventListener('click', () => {
        ...
        document.getElementById('cpwd-submit')?.addEventListener('click', async () => {
            ...
            if (!oldPwd) { cpwdError.textContent = '请输入当前密码'; cpwdError.style.color = '#ff4444'; return; }
            if (!newPwd) { cpwdError.textContent = '请输入新密码'; cpwdError.style.color = '#ff4444'; return; }
            if (newPwd.length < 6) { cpwdError.textContent = '新密码至少6个字符'; cpwdError.style.color = '#ff4444'; return; }
            if (oldPwd === newPwd) { cpwdError.textContent = '新密码不能与旧密码相同'; cpwdError.style.color = '#ff4444'; return; }

            cpwdError.textContent = '验证中...';
            cpwdError.style.color = '#00d4ff';

            try {
                await authChangePassword(this.currentUsername, oldPwd, newPwd);
                cpwdError.textContent = '密码修改成功！';
                cpwdError.style.color = '#00FF44';
                setTimeout(() => {
                    document.getElementById('change-pwd-modal').classList.add('hidden');
                    document.getElementById('modal-overlay').classList.add('hidden');
                    document.getElementById('cpwd-old').value = '';
                ...
                cpwdError.textContent = e.message || '修改失败，请重试';
                cpwdError.style.color = '#ff4444';
            }
        });

        // 退出登录
        document.getElementById('cpwd-logout')?.addEventListener('click', async () => {
            try { await authLogout(); } catch (e) { /* ignore */ }
            // 清除本地解锁缓存，防止污染其他账号
            localStorage.removeItem('os2_unlocked_planes');
            localStorage.removeItem('os2_unlocked_wingmen');
            localStorage.removeItem('os2_max_cleared_level');
            localStorage.removeItem('os2_dev_mode');
            // 重置内存数据，防止污染下一个登录账号
            this.unlockedPlanes = ['Ranger'];
            this.unlockedWingmen = ['none'];
            this.maxClearedLevel = 0;
            this.selectedPlane = 'Ranger';
            this.selectedWingman = 'none';
            this.currentUsername = null;
            this.isGuestMode = false;
            document.getElementById('change-pwd-modal').classList.add('hidden');
            document.getElementById('modal-overlay').classList.add('hidden');
        ...
        document.getElementById('view-data-btn')?.addEventListener('click', () => {
            const content = document.getElementById('data-view-content');
            const cfg = this.AIRCRAFT_CONFIG;

            let html = `<p style="color:#fff">👤 用户名：<strong style="color:#00d4ff">${this.currentUsername || '游客'}</strong></p>`;
            html += `<p style="color:#fff">🏆 最高通关：<strong style="color:#ffaa00">第 ${this.maxClearedLevel} 关</strong> / 共 10 关</p>`;
            html += `<hr style="border-color: rgba(0,212,255,0.3); margin: 15px 0;">`;

            html += `<p style="color:#00d4ff;">已解锁战机 (${this.unlockedPlanes.length}/${Object.keys(cfg.planes).length})：</p>`;
            for (const [key, p] of Object.entries(cfg.planes)) {
                const ok = this.unlockedPlanes.includes(key);
                html += `<p style="color:#fff">${ok ? '✅' : '🔒'} ${p.name}${ok ? '' : `（通关第${p.unlockLevel}关解锁）`}</p>`;
            }

            const wingmenNoNone = Object.entries(cfg.wingmen).filter(([k]) => k !== 'none');
            const unlockedWingmenNoNone = this.unlockedWingmen.filter(k => k !== 'none');
            html += `<p style="color:#00d4ff; margin-top:10px;">已解锁僚机 (${unlockedWingmenNoNone.length}/${wingmenNoNone.length})：</p>`;
            for (const [key, w] of wingmenNoNone) {
                const ok = this.unlockedWingmen.includes(key);
                html += `<p style="color:#fff">${ok ? '✅' : '🔒'} ${w.name}${ok ? '' : `（通关第${w.unlockLevel}关解锁）`}</p>`;
            }
        ...
        document.getElementById('data-view-close')?.addEventListener('click', () => {
    ...
    async handleLevelVictory() {
        ...
        // 同步到云端（已登录时）— 不阻塞 UI 渲染
        if (localStorage.getItem('os2_dev_mode') !== 'true') {
            saveToCloud({
                unlocked_planes: this.unlockedPlanes,
                unlocked_wingmen: this.unlockedWingmen,
                max_cleared_level: this.maxClearedLevel
            }).catch(e => console.warn('[SYNC] 云端保存失败:', e));
        }
