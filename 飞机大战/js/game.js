class Game {
    constructor() {
        ...
        this.bgNebulaScrollY = 0;
        this.bgNebulaScrollSpeed = 0.2;
    ...
    handleLogin() {
        ...
        if (account === '0001' && password === '1011') {
            msg.style.color = '#44ff44';
            msg.textContent = '验证通过！解锁所有内容...';
            ...
            setTimeout(() => {
                ...
                // Refresh UI
                this.updateCurrencyDisplay();
                alert('开发者模式已激活：所有战机、僚机、关卡已解锁，获得无限资源。');
            }, 1000);
        } else {
    ...
    unlockAllContent() {
        // 1. Max Currency
        economySystem.gold = 9999999;
        economySystem.diamond = 999999;
        economySystem.saveToStorage();
        
        // 2. Unlock All Aircraft
        const allAircraft = Object.keys(AIRCRAFT_TYPES).map(k => AIRCRAFT_TYPES[k].id);
        aircraftManager.unlockedAircraft = allAircraft;
        // Set all levels to max (e.g., 30 for super stats)
        allAircraft.forEach(id => {
            aircraftManager.aircraftLevels[id] = 30; 
        });
        aircraftManager.saveToStorage();
        
        // 3. Unlock All Wingmen
        const allWingmen = Object.keys(WINGMAN_TYPES).map(k => WINGMAN_TYPES[k].id);
        wingmanManager.owned = allWingmen;
        wingmanManager.saveToStorage();
        
        // 4. Unlock All Levels
        for (let i = 1; i <= 10; i++) {
            campaignManager.levelProgress[i] = { stars: 3, score: 10000 };
        }
        campaignManager.maxLevel = 10;
        campaignManager.saveToStorage();
        
        // 5. Unlock Achievements
        Object.keys(achievementSystem.achievements).forEach(key => {
            achievementSystem.achievements[key].unlocked = true;
        });
        achievementSystem.saveToStorage();
    }
    
    handleKeyDown(e) {
        // Allow typing in login inputs
    ...
    showAchievementScreen() {
        ...
        document.getElementById('achievement-level').textContent = achievementSystem.level;
        
        const list = document.getElementById('achievement-list');
        list.innerHTML = '';
        
        // Convert achievements object to array and sort
        const achievements = Object.values(achievementSystem.achievements).sort((a, b) => {
        ...
        achievements.forEach(ach => {
            ...
            if (ach.unlocked) {
                ...
                const current = achievementSystem.progress[ach.reqType] || 0;
                progress = Math.min(100, (current / ach.reqValue) * 100);
            }
    ...
    showHangarScreen() {
        ...
        // Default to aircraft tab, make sure button state reflects this
        this.switchHangarTab('aircraft');
    }
    ...
    renderHangarItems(tab) {
        ...
        if (tab === 'aircraft') {
            Object.values(AIRCRAFT_TYPES).forEach(aircraft => {
                ...
                const isSelected = aircraftManager.selectedAircraft === aircraft.id;
                
                const card = document.createElement('div');
                card.className = `item-card ${isOwned ? 'owned' : ''} ${isSelected ? 'selected' : ''}`;
                ...
                const imgKey = aircraftImageMap[aircraft.id] || 'p1';
                const imgSrc = window.ASSET_SOURCES[imgKey];

                card.innerHTML = `
                    <div class="item-icon-container">
                        <img src="${imgSrc}" class="item-icon" alt="${aircraft.name}">
                    </div>
                    <div class="item-name" style="color: ${aircraft.color}">${aircraft.name}</div>
                    <div class="item-price">${isSelected ? '当前使用' : (isOwned ? '已拥有' : '未解锁')}</div>
                `;
                
                card.addEventListener('click', () => {
                    if (isOwned && !isSelected) {
                        aircraftManager.select(aircraft.id);
                        this.renderHangarItems(tab);
                    } else if (!isOwned) {
                        // Maybe redirect to shop?
                        alert('请前往商店解锁此战机');
                    }
                });
            ...
            Object.values(WINGMAN_TYPES).forEach(wingman => {
                ...
                const imgKey = wingmanMap[wingman.id] || 'w1';
                const imgSrc = window.ASSET_SOURCES[imgKey];

                card.innerHTML = `
                    <div class="item-icon-container">
                         <img src="${imgSrc}" class="item-icon" alt="${wingman.name}">
                    </div>
    ...
    startGame() {
        ...
        // Initialize difficulty system if not exists (handle global instance)
        this.difficultySystem = window.difficultySystem || new DifficultySystem();
    ...
    update() {
        ...
        const healthPercent = this.player ? this.player.health / this.player.maxHealth : 0;
        difficultySystem.update(this.deltaTime, healthPercent);
        this.spawnSystem.setDifficulty(difficultySystem.dynamicDifficulty > 1.5 ? 'hard' : (difficultySystem.dynamicDifficulty > 1.2 ? 'normal' : 'easy')); // Simplistic mapping, better to pass multiplier directly
        
        // Update Effects
        effectsSystem.update(this.deltaTime);
    ...
    updateStars() {
        ...
        if (this.bgScrollY >= this.canvasHeight) this.bgScrollY = 0;
        
        this.bgNebulaScrollY += this.bgNebulaScrollSpeed;
        if (this.bgNebulaScrollY >= this.canvasHeight) this.bgNebulaScrollY = 0;
    ...
    updatePlayer() {
        ...
        // If 2-Player mode, P1 only uses WASD
        if (this.gameMode === GAME_MODES.COOP) {
            ...
            // Classic mode: Arrow keys OR WASD
    ...
    handleEnemyBulletPlayerCollision(collision) {
        ...
        if (player.takeDamage(GAME_CONFIG.BULLET.ENEMY.DAMAGE)) {
            ...
            // Effects
            effectsSystem.shake(10, 0.3);
            effectsSystem.flash('rgba(255, 0, 0, 0.3)', 0.2);
            difficultySystem.onPlayerHit();
    ...
    handlePlayerEnemyCollision(collision) {
        ...
        // Ramming damage:
        // Enemy takes player's max health (guaranteed kill for non-bosses usually)
        enemy.takeDamage(player.maxHealth || 100);
        
        particleSystem.createExplosion(enemy.x, enemy.y, 1);
        soundManager.playExplosion();
        
        // Player takes damage (can be reduced by defense)
        if (player.takeDamage(30)) {
            particleSystem.createExplosion(player.x, player.y, 1.5);
            
            // Effects
            effectsSystem.shake(15, 0.4);
            effectsSystem.flash('rgba(255, 0, 0, 0.5)', 0.3);
            difficultySystem.onPlayerHit();
    ...
    handlePlayerBossCollision(collision) {
        ...
        if (player.takeDamage(50)) {
            ...
            // Effects
            effectsSystem.shake(20, 0.5);
            effectsSystem.flash('rgba(255, 0, 0, 0.6)', 0.4);
            difficultySystem.onPlayerHit();
    ...
    handlePowerupPlayerCollision(collision) {
        ...
        let color = '#ffffff';
        switch (powerup.type) {
            case 'health': text = 'HP +30'; color = '#44ff44'; break;
            case 'power': text = 'POWER UP!'; color = '#ffff44'; break;
            case 'shield': text = 'SHIELD!'; color = '#44ffff'; break;
        }
        
        if (text) {
            // Simple floating text (implemented via particle system or DOM)
            // For now, let's use a simple DOM element or particle if available
            // Assuming particleSystem can handle text or we create a temp element
            const floatText = document.createElement('div');
            floatText.textContent = text;
            floatText.style.position = 'absolute';
            floatText.style.left = (player.x + this.canvas.getBoundingClientRect().left) + 'px';
            floatText.style.top = (player.y + this.canvas.getBoundingClientRect().top) + 'px';
            floatText.style.color = color;
            floatText.style.fontWeight = 'bold';
            floatText.style.fontSize = '20px';
            floatText.style.textShadow = '0 0 5px #000';
    ...
    render() {
        ...
        // Effects Pre-render (Shake)
        effectsSystem.preRender(this.ctx);
        ...
        // Effects Post-render (Flash, restore context)
        effectsSystem.postRender(this.ctx);
    }
    
    renderBackground() {
        const gradient = this.ctx.createLinearGradient(0, 0, 0, this.canvasHeight);
        
        // Dynamic atmosphere based on tension
        let tension = 0;
        if (typeof difficultySystem !== 'undefined') {
            tension = difficultySystem.tension;
        }
        
        // Interpolate color based on tension (0 = Blue, 100 = Reddish)
        const r1 = Math.floor(10 + (32 * tension / 100));
        const g1 = 10;
        const b1 = Math.floor(26 - (16 * tension / 100));
        
        const r2 = Math.floor(26 + (36 * tension / 100));
        const g2 = 26;
        const b2 = Math.floor(62 - (36 * tension / 100));
        ...
        const nebulaImg = window.imageLoader.get('bg_nebula');
        if (nebulaImg) {
            this.ctx.globalAlpha = 0.5; // Blend with gradient
            // Draw twice for seamless scrolling
            this.ctx.drawImage(nebulaImg, 0, this.bgNebulaScrollY, this.canvasWidth, this.canvasHeight);
            this.ctx.drawImage(nebulaImg, 0, this.bgNebulaScrollY - this.canvasHeight, this.canvasWidth, this.canvasHeight);
            this.ctx.globalAlpha = 1;
        }

        // Draw Stars Background
        const starsImg = window.imageLoader.get('bg_stars');
        if (starsImg) {
            this.ctx.globalCompositeOperation = 'screen'; // Additive blending for stars
            // Draw twice for seamless scrolling
            this.ctx.drawImage(starsImg, 0, this.bgScrollY, this.canvasWidth, this.canvasHeight);
            this.ctx.drawImage(starsImg, 0, this.bgScrollY - this.canvasHeight, this.canvasWidth, this.canvasHeight);
            this.ctx.globalCompositeOperation = 'source-over';
        } else {
            // Fallback to procedural stars if image not found
...
window.onload = () => {
    // Show loading text if possible, or just wait
    console.log('Loading assets...');
    window.imageLoader.load(window.ASSET_SOURCES);
    
    window.imageLoader.onComplete = () => {
