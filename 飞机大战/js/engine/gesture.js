...
class OneEuroFilter {
    ...
    filter(a,b){null==this.f&&(this.f=a,this.h=0,this.g=b);const c=b-this.g||.016,d=this.i(c,this.e),e=(a-this.f)/c,f=d*e+(1-d)*this.h,g=this.c+this.d*Math.abs(f),j=this.i(c,g);return a=j*a+(1-j)*this.f,this.f=a,this.h=f,this.g=b,a}
...
export default class GestureEngine {
    constructor() {
        ...
        // 优化滤波器参数，提高高速移动时的响应 (beta 提高到 0.05)
        this.filters = Array(21).fill(null).map(() => ({ x: new OneEuroFilter(1,.05), y: new OneEuroFilter(1,.05), z: new OneEuroFilter(1,.05) }));
        ...
        this.detectInterval = 33.3; // 限制最高 30 FPS
    }
    ...
    analyzeHand(landmarks, handedness) {
        ...
        if (indexStraight && middleStraight) {
            ...
            if (isLeftHand) {
                if (indexStraight || (distIndex > distMiddle * 1.02)) newState = 'GUN';
                else newState = 'FIST';
            } else {
                if (indexStraight || (distIndex > distMiddle * 1.1)) newState = 'GUN';
                else newState = 'FIST';
            }
        }
